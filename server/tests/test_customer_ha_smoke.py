...
def test_customer_desktop_build_is_the_sole_default_target() -> None:
    ...
    assert base_config["version"] == "2.0.3"
    ...
    assert 'name = "video-replica-desktop"\nversion = "2.0.3"' in cargo_lock
    assert 'version = "2.0.3"' in cargo_toml.split("[lib]", maxsplit=1)[0]
    assert 'version = "2.0.3"' in server_package.split("[project]", maxsplit=1)[1]
    assert 'version="2.0.3"' in server_main
