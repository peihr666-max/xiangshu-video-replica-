...
# HEAD_REVISION 取本分支链尾 083：本分支 = main(→081) + CW-067 083_recharge_orders_multi_provider，
# 迁移后 alembic 版本头即 083，9 处 assert version == HEAD_REVISION 依赖此值。
...
def test_pg_billing_constraints_downgrade_guard() -> None:
    ...
    try:
        ...
        with pytest.raises(RuntimeError, match="cannot downgrade 026"):
            # Fifteen steps from head: 083->081 (empty wechat columns,
        ...
        # (083->081->038->037->036->035->034->033->032->029->028->031->027->026->025)
