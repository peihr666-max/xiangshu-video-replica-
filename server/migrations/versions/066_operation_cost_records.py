"""066_operation_cost_records — freeze rates and record actual provider usage.
...
revision = "066_operation_cost_records"
down_revision = "065_second_based_billing"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        has_records = bind.execute(sa.text("SELECT 1 FROM operation_cost_records LIMIT 1")).first()
        has_snapshots = bind.execute(
            sa.text(
                "SELECT 1 FROM generation_tasks WHERE "
                "cost_rate_subject_snapshot IS NOT NULL OR "
                "cost_unit_price_fen_snapshot IS NOT NULL OR "
                "external_unit_price_fen_snapshot IS NOT NULL OR "
                "actual_output_seconds IS NOT NULL OR cost_status IS NOT NULL LIMIT 1"
            )
        ).first()
        if has_records or has_snapshots:
            raise RuntimeError(
                "cannot downgrade 066 while operation cost records or task cost snapshots exist"
            )
    if bind.dialect.name == "postgresql":
