...
export function BillingRatesManager({
  ...
  async function save(event: React.FormEvent) {
    ...
    if (
      ...
      setError(
        "请填写非负单价（最多 6 位小数）和调整原因；启用收费必须填写售价。",
      );
      return;
    }
    const payload = {
      ...
      tariff: {
        enabled,
        unit_credits: price || null,
        unit_cost_fen: cost || null,
  ...
  return (
    <section className="admin-panel" aria-label="逐项成本与售价">
      <h2>逐项成本与售价</h2>
      <p>
        管理员启用并填写售价后，用户按每项实际用量扣分。未配置、停用或零售价均由平台承担；成本留空表示待核对。
      </p>
      <p>
        成本以分为单位，支持 6 位小数；售价以积分为单位。云存储和 ZPay
        按零费用核算。
      </p>
      {error && (
        <p role="alert">
            ...
            <tbody>
              {catalog.services.map((service) => (
                <tr key={service.service}>
                  ...
                  <td>{billingUnit[service.unit]}</td>
                  <td>
                    {service.tariff.unit_cost_fen ??
                      (service.service === "cos" || service.service === "zpay"
                        ? "0"
                        : "待配置")}
                  </td>
                  <td>
                    {service.tariff.enabled ? service.tariff.unit_credits : "0"}
                  </td>
                  <td>
      ...
      {selected && (
        <form onSubmit={save} aria-label={`配置 ${selected.name}`}>
          <h3>
            {selected.name} · 每{billingUnit[selected.unit]}
          </h3>
          <label>
            成本（分 / {billingUnit[selected.unit]}，留空待核对）
            <input
              value={cost}
          ...
          <label>
            售价（积分 / {billingUnit[selected.unit]}）
            <input
              value={price}
              onChange={(event) => setPrice(event.target.value)}
              inputMode="decimal"
              disabled={busy || readOnly || !selected.customer_charge_allowed}
            />
          </label>
          <label>
            <input
          ...
          {selected.unit === "second" && (
            <label>
              ...
              <select
                value={rounding}
                onChange={(event) =>
                  setRounding(event.target.value as "ceil" | "exact")
                }
                disabled={busy || readOnly}
              >
                <option value="ceil">每次请求向上取整到秒</option>
