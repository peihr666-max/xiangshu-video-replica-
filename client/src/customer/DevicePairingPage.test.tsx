...
describe("DevicePairingPage (FE-03 / T30)", () => {
  ...
  it("validates required fields are not empty", () => {
    ...
    expect(screen.getByLabelText(/device fingerprint/i)).toHaveAttribute("required");
  ...
  it("calls onError callback when enrollment fails", async () => {
    ...
    try {
      ...
      global.fetch = vi.fn().mockResolvedValueOnce({
        ok: false,
        status: 400,
        json: vi.fn().mockResolvedValueOnce({ message: "Invalid activation code" }),
      });
  ...
  it("does not expose any plaintext secrets or tokens in UI", () => {
    ...
    expect(screen.queryByLabelText(/activation-code-secret/i)).not.toBeInTheDocument();
