...
export function AnalysisWorkspace({
  ...
  function updateShotCard(index: number, nextShot: ShotCard) {
    if (readOnly || isSaving || isWorkspaceBusyRef.current) return;
    const nextShots = shots.map((shot, shotIndex) =>
      shotIndex === index ? nextShot : shot,
    );
    setShots(nextShots);
    setSaveMessage("");
    setShotCardsDirty(!shotCardsEqual(nextShots, savedShotsRef.current));
  }
      ...
      content: (
        <div className="analysis-primary">
          <section className="stage-block" aria-label="动作时间段与口播">
            ...
            >
              {isLoading || generationDrafts.isLoading ? (
