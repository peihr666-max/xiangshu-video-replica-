...
| ⑦ | 冻结值的**说明性注释**与代码脱节：`test_cw056:125-131` 声称 HEAD_SCHEMA_COUNTS/DIGEST 仍冻结在 083 基线（tables=77/columns=913）、「B 组预期红」，实测字面量早已随 #64 更新到 089 的真实值（78/923）且全绿。同日另一起：据 `migrate.sh` 源码判断「多头会静默取第一个 head」，实测 `:28` 在多脚本头下**硬失败**，真正缺口在 `:36` 的库侧多行 `alembic_version` | MIGRATION-GUARD-20260912（#69，475f995）；`test_cw056_supported_head_matrix.py:125-131`、`deploy/postgres/migrate.sh:35-36` | 冻结值/常量更新后，**解释它的注释没有同步**；下一位读者（人或 AI）据注释判断会得出与事实相反的结论——更糟的是，「注释说预期红」会让真红被当成已知状态放过 | §4/§5.1：冻结值的注释**不写具体数字**（数字会漂），要写就写「由 `--print-schema` 生成」；工具化后这类注释不再需要人工维护 |
| ⑧ | 断言锚点可被**重新记录洗白**：初版 `--record` 会把 `naming_policy.adoption_head` 前移到当前 head，于是「加一个违规命名的迁移 → 跑 `--record` → 提交」让违规成为锚点本身、落进豁免集，此后所有检查都通过 | MIGRATION-GUARD-20260912 实现期自曝，**提交前修掉**（#69）；`scripts/ci/migration_manifest.py` 的 `NAMING_POLICY_ADOPTION_HEAD`、`test_migration_guard_manifest.py::test_the_naming_anchor_cannot_be_laundered_by_re_recording` | 策略的**锚点/基线存在被自己生成的产物里**，而生成动作又会前移它——检查者与被检查者是同一个 | §5.1：锚点必须是**代码里的常量**，检查时断言产物记录的锚点等于该常量；锚点未知要 fail-closed 报错，不能静默放行（「策略失效」与「没有违规」必须可区分）。**推广**：任何「豁免清单 + 自动生成」的组合都适用 |

**共性**：没有一起是 alembic/CI 的 bug，但分两类——

- **①~⑥ 并行写入共享状态**时缺少合并协议。本手册 §2/§3 就是那份协议。
- **⑦⑧ 验证态与它的描述/产物漂移**：⑦ 是注释与代码脱节，⑧ 是检查的锚点与被检查的产物同源。这两类的共同修法是**让机器重算，别让人抄写，也别让策略自己给自己定基线**——即 §5.1 的工具化方向。⑦ 还说明一件事：注释里的具体数字是有保质期的负债，能生成就不要写死。

## 7. 边界与红线（不因本手册改变）

- 迁移文件名冻结（`025_...`~`030_...` 为客户版冻结段；082+ 为增量段）、已发布 revision 逐字节不可改——重挂 `down_revision` 仅适用于**尚未合并进 main** 的分支迁移。**存量文件一律不改名**；自 `MIGRATION-GUARD-20260912` 起**新增**迁移改用时间戳前缀（`YYYYMMDDTHHMM_<slug>`，见 §5.1），两者不冲突——链的顺序由 `down_revision` 决定，文件名只是标签（main 现链 `081→082→086→083→089` 早已不是数字序）。
