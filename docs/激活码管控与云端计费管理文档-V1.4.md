...
| COS 生命周期策略 | 否，仅读状态 | 自动过期规则已取消；人物、项目素材与成片永久保存 |
...
- 人物身份/角色图片使用 `users/{owner_user_id}/identities/` 与 `users/{owner_user_id}/personas/`，永久保存；
- 源视频、首帧等项目素材沿用 `projects/{project_id}/...`，永久保存；
- 成片沿用 `generation-results/{project_id}/...`，永久保存；
- 用户素材库上传使用 `materials/{owner_user_id}/{asset_id}/...`，永久保存；
- COS 桶不配置自动过期规则；后续若需删除对象，必须由有审计记录的显式业务动作完成；
...
- 单 license 并发；
- 各免费动作日配额；
- 永久保存带来的 COS 容量与流量成本需进入运营预算；如未来调整保留期限，必须重新取得业务方决策并设计存量迁移；
- 充值凭证类型与财务审核人。
