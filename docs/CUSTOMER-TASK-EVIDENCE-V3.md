...
本地短音频以 Base64 直传，云存储保留签名 URL；修正 Flash 必填音频格式和采样率类型，预检先于付费尝试。专项 35 passed；完整 Linux 静态门通过（1418 前端）；93 个测试文件的全量 PG 2046 passed / 1 既有 skip。[证据](evidence/FIX-ASR-400-20260914.md)与[同类问题排查](媒体云端可访问性排查-20260914.md)区分代码修复、环境准备和真实付费验收。
