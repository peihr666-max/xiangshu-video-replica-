...
## CW-019 — 拆出客户与管理员独立构建制品（W3，2026-09-10）

分支 `feat/customer-v3-cw019-split-build-artifacts`，基线 `origin/main@5e9d2d7`，收尾时**两次** rebase：先到 `origin/main@d8f3352`（CW-016/025/033 已合入，`package.json` 与 `docs/客户版任务清单-V3.md` 两处交集均自动合并无冲突），再到 `origin/main@e06b13c`（CI 重构 #8 + CW-017 #9，同为 0 冲突，但 ci.yml 的自动合并结果语义破损、已手工修，见下）。Vite MPA 双入口 + 双 outDir 物理分离：客户 `client/dist`（outDir 名被 `tauri.conf.json` 的 `frontendDist: "../dist"` 依赖，`tauri.customer.conf.json` 无 `build` 键而继承同一值，故不可改名）、管理 `client/dist-admin`（`base: "/admin/"`）。客户 JS 901.67→747.04 kB，管理 JS 独立 365.44 kB；`AdminApp.tsx` 组件本体零改动、`src/admin/**` 的唯一改动是 `SystemSettingsPage.tsx` 注入 `controlBackend`（依赖注入方案的必然调用点；早先登记为「`AdminApp` 与 `src/admin/**` 零改造」，已在 CW-019-EVIDENCE.md「Admin Surface Regression」节纪正），管理域 **22 文件 / 152 用例**（`src/admin/**` 20 文件 119 + `AdminApp.test.tsx` 19 + `api.admin.test.ts` 14）全绿（早先登记为 18 文件 117 / 合计 150，属**计数误计而非回归**：差额恰为 `CostDetails.test.tsx` 与 `CustomersManagementPage.test.tsx` 两个单用例文件，二者在开工基线即存在，基线→HEAD 之间管理域无测试文件增删）。双层排除断言承接 CW-011：源码级 `client/src/entryContract.test.ts`（7 用例，秒级）+ 产物级 `scripts/verify_customer_bundle.mjs`（CI 独立 step）。实测 Vite 8.2.1 / Rolldown 1.2.4 **忽略全部** `build.rollupOptions.treeshake` 写法（`sideEffects` hint / `moduleSideEffects` 函数式 / 全局 `false` 三者产物哈希字节级相同），构建层排除不可行 → 改依赖注入切断 `CustomerWorkspace → StudioWorkspace → SettingsPanel` 的 `/api/control/` 静态可达链，并清理已证无效的 treeshake 块与 `sideEffects` 字段。红→绿：`entryContract` 4 failed/1 passed → 7 passed；旧单入口 901.67 kB chunk 被 verify 判 ❌（命中 `/api/control/`、`激活码批次`、`审计中心`、`强制下线`）→ ✅ 17 文件 0 命中、退出码 0。两次 `build:all` 的 21 文件 SHA-256 清单 diff 为空（可复现）。

新增**阳性对照**机制封堵一类伪绿：`minify: "oxc"` 会 mangle 标识符，`AdminApp`/`exchangeAdminSession`/`internalAccessToken` 等形状串在压缩产物中恒不命中，「断言静默失效」与「制品真的干净」原本在输出上完全同形；现要求 6 条管理域字面量在 `client/dist-admin` 各命中 ≥1 文件，否则 exit 1（实测 6/6），并以变异测试（注入 `import App from "./App"` + `<App/>` → exit 1，还原后 7/7 绿）证明「匹配前剔注释」未把源码级断言掏空。`build.copyPublicDir: false` 断开 `client/public/studio/` 约 25MB 客户专用素材向管理制品的无评审隐式搬运（`dist-admin` 27MB→388KB），改 `favicon.svg` 单文件显式拷贝。CodeReview 子代理 1M+4m+4n：**M1 已修**（`deploy/customer-git-rollout.sh` 是仓库唯一客户站部署路径，原先不产出/不部署/不备份 `dist-admin`，合并后首次发布会打印 SUCCESS 而 `/admin` 静默 500 → 现同 SHA 原子替换 + `admin-site-before.tar.gz` 备份/回滚 + `build:all`+verify 纳入发布链 + `curl /admin/` 真实探活）、**m1–m4 全部已修**、**n2/n3/n4 已修**、**n1 与「`biome.json` 的 `files.includes` 不覆盖 `scripts/`」登记 follow-up 不修**（n1 触碰 §5.4 红线保护的 `allow` 内网三段 + `deny all` + `auth_basic` 安全块且现有管控已兜底；biome 覆盖面属仓库级配置变更超出本任务范围）。收尾期另抓到并修复一处**本任务造成的确定性真回归**：M1 把发布链构建命令改为 `build:all` + `verify:customer-bundle`，而上游守卫 `server/tests/test_customer_git_rollout.py` 断言的是旧字面量 `npm run build --workspace client`（纯静态文本断言，隔离复跑 1 failed/3 passed，**100% 可复现**）；该失败此前被 client vitest 时序 flake **完全掩盖**——根 `package.json` 的 `check` 以 `&&` 串联 8 段且 client 段排在 server pytest 之前，flake 短路使 pytest 一次都没执行到，故改**分段执行**（每段独立记退出码）后暴露。处置为修测试 needle（不回退必要的 M1 脚本改动）并新增 `test_customer_git_rollout_ships_admin_artifact_in_the_same_release_run` 锁住 M1 六项行为，现 4 passed + ruff 双通过；同轮核实上游守卫 `test_internal_admin.py` 按精确字面量 `location ^~ /admin {` 定位 internal-p0 安全块，既证明本任务未削弱该块（隔离验证通过），也构成 n1 不修的硬约束。分段门禁结果：client 80 文件 1292 passed、biome/e2e/tauri/mypy 全 0。第二次 rebase 另暴露一处「0 冲突 ≠ 语义正确」：#8 给 ci.yml 的 `Build web client` 加了 `if: needs.changes.outputs.desktop == 'true'`，而本任务新增的两个 step（`Build admin bundle` / `Verify customer bundle…`）是针对旧 ci.yml 写的、无任何门控，git 自动合并后只改 `server/**` 的 PR 上 `client/dist` 不存在，verify 会按设计（缺制品必须失败，否则「没构建」被当成「干净」）exit 1 致 CI 必红；已补齐与上游逐字相同的门控，并核实 #8 新增的 `test_build_contracts.py` 全部严格计数断言（`persist-credentials: false`/`fork_pr_guard`/`runs-on`/`LOCAL_ARTIFACT_ROOT`/`SHA256SUMS.txt`/`checkout@`/`setup-node@`）不被本任务插入的 step 破坏，守卫测试 23 passed。**树 `485541f`（本轮 docs 纪正前的提交树；push 树与其仅差 docs-only 编辑）上两条门禁路径均绿**（#8 已把 CI 的 Linux 质量门拆成 `check:static` + `run-pytest-shards.sh`，4 片各带独立 PG 容器，并在 AGENTS.md §4 将「push 前本地跑绿」升级为强制）：路径 B `npm run check:sharded` SHARDED_EXIT=0（21:00:54→21:08:50，7m56s；client 80 文件 1295 用例 24.96s、4 片 pytest 645+529+511(+1 skipped)+558 = **2243 passed / 1 skipped / 0 failed**、secrets/biome/tsc/cargo/ruff/mypy 全绿）；同一命令在前一轮树 `bc8bca2` 上亦 EXIT=0（9m32s，4 片用例数逐一相同）；路径 A 顺序全量 `npm run check` 分段执行全 EXIT=0（pytest 2243 passed / 1 skipped in 1280.98s），两次运行服务端用例数逐一相同。收尾期另**主动纪正本任务证据里两处自己写错的事实**（均在最终树上重新取证，见 CW-019-EVIDENCE.md「Admin Surface Regression」节）：一是「`src/admin/**` 全部页面零改动」不实（唯一改动是 `SystemSettingsPage.tsx` 注入 `controlBackend`；`AdminApp.tsx` 本体确实零改动），二是管理域计数 150 应为 **152**（差额是两个单用例文件被早先的文件过滤漏掉，二者开工基线即存在，故 152 对整个任务期成立，**不是回归**）。docs 每轮编辑后均复跑 9 个读 docs 的 server 测试文件（**128 passed**）+ `verify_no_secrets.sh`（EXIT=0）。制品在最终树上重建并复验：两次 `build:all` REPRO=IDENTICAL（21 文件），客户 JS 746.75 kB（`index-DhDiBhwK.js`，因 CW-017 改动客户包输入而变），管理 JS 365.44 kB（`admin-Dp4WSijI.js`，**哈希未变**——两制品物理隔离的旁证），verify 扫描 17 文件 0 命中 + 阳性对照 6/6 + VERIFY_EXIT=0。§5.6 归属拆分：不动 CI 的 `Verify customer installer excludes local launchers`、不动 Cargo feature、不动 tauri conf——制品检测扩大归 CW-021、feature/config 唯一默认归 CW-020、installer hooks 与签名发布归 CW-024。

证据层级 `AUTOMATED_VERIFIED`；`deploy/customer-git-rollout.sh` 未在目标机执行（仅 `bash -n`，本机无 shellcheck），nginx 未真实 reload、`/admin` 未真实浏览器探活，未过真实链路故不提升 `STAGING_VERIFIED`。`cargo test`、`npm audit`、客户浏览器 E2E、Windows Tauri/NSIS 门禁本地跑不到，以 push 后三门禁为准。完整 §14 记录、两份制品 SHA-256 清单、红绿原始输出与逐条 finding 处置见 `docs/evidence/CW-019-EVIDENCE.md`。
