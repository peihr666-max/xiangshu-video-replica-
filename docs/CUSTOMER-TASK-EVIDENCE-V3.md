...
## 2026-09-15 14:38 — 2K双条及完整音频口播完成，R68标题修复

- 首尾帧2K9:16批次 fc816f33-a777-4316-bccc-6f354f76ecb8 的两条4秒任务均成功。任务1350c22d-dbc8-4eaa-bd76-597adbda2e4a于14:15:41、d35987d4-9468-4f26-92d5-6d225924177e于14:15:34完成，耗时约12分08秒/12分01秒，各实扣48积分。前端分别选择结果并完整播放到结束，均4.458333秒、1463×2560、无媒体错误；第一条保存到素材库成功，第二条点击下载收到浏览器下载事件。未取得本地下载路径，不能声称本地文件哈希已验收。
- 两条成片已归档COS：cfdc4dae-ed27-407d-b569-86cd2d4d3393（10883628字节，SHA256 582c5b0440e46cfebce759107af18143cf58b7a43cd8a12e5cbbfa7fe0699ee3）；30935471-8b9e-4b7e-b88b-1d8ff6ca492f（10759998字节，SHA256 86c9672a18aecf09481b29cab8d8cd26979f20f33c460ca6b540b43e6872b0c4）。实际像素与选择2K规格分开记录，不冒称精确1440×2560。
- 真实完整音频口播：素材库“联调合成声音.mp3”资产98d42ead-695f-47f6-ad1c-0de702a7223f（24.013197秒），配已有授权场景口播分身，任务4c3ad96a-81db-46c3-9589-1fb0c30ca816，mode=AUDIO，供应商Hifly回执_a31uTQBnzTQmWc8GwABYg。14:13:09提交，等待共享渲染通道后14:16:08完成；预扣50、实扣48、退2。前端完整播放24秒、864×1040、无媒体错误。没有听觉证据，音色和口型精度仍不能判定通过。本次未成功执行取消操作，不将其计入取消路径验收。
- 口播成片873ded6c-90d1-4382-a7df-7cfb3dc4b1df已归档，4102034字节，SHA256 0a14c8e884f846d214e8b21c7c3c4e736c09b0acadef8231335d08b58e38471b。
- R68：独立视频任务未命名时以请求快照的提示词摘要显示，列表及详情一致，优先保留手动标题；仅计算返回值，不改写历史快照或数据库标题。真实从个人消费明细进入任务详情、再到任务中心验证历史任务标题生效。PG独立创建24项、Ruff及Mypy147文件通过；开发者自检完成，不冒称独立评审。
- 个人中心与后端账本一致：累计消费815/1000、可用185、待结算0。已核对ASR两次各24、旧失败138全退、2K两次各48、完整音频口播48及2积分退回。管理端仍明确显示会话失效，已提出登录请求，新增三类业务的管理端明细及临时H3扩展模式恢复OFF尚待完成。
- 供应商记录：秘塔827.5H3点（12条请求，含两条2K各68）；飞影14:15新增数字人创作217点，本轮已识别共1026点。不同供应商单位不合并为现金；保守人民币预算占用70/100元并非实际支出总账，Apilio/ASR等人民币核账仍待完成。

未完成项继续保留：严格“只换人物、其余不变”的R2V质量不合格；输入格式/累计时长边界、参考素材详情展示、取消等异常路径、视频号最后1/150统计及视频号云归档/前端可见、供应商现金账单、管理端新明细和测试开关恢复。R2V严格保持场景建议人物区域跟踪与局部替换后合成原背景/原音轨，当前未实现；不能以接口成功替代画面质量验收。原始证据148—152保留本机。PR #109按已验证代码交付，不代表全部业务完成，未部署。

## 2026-09-15 14:10 — 首尾帧与双条任务增量（R66—R67）

- R66：真实选择尾帧后草稿已保存物理资产ID，但选择器局部签名预览未传回创作页，导致尾帧空白。选择时把已解析的素材预览合并到工作区，草稿仍只保存资产ID，重登仍重新校验归属及获取预览。真实恢复首尾帧均可见；针对“选择后立即预览”回归先失败后通过，工作区96项、类型及Biome通过。
- R67：两条视频后台均RUNNING，列表却显示排队中/取消任务。批次未完成时错误沿用旧QUEUED状态；现在根据子任务SUBMITTING/供应商QUEUED/RUNNING/ARCHIVING及部分完成状态报告RUNNING，保留不确定状态与终态规则。四类PG回归先失败后通过；独立创建与Worker合计73项、Ruff/Mypy147文件通过。只重启独立API后，同一在途真实批次已自动显示生成中/查看详情，Worker未停止，没有重发供应商任务。

首尾帧+2K+双条真实测试进行中：批次fc816f33-a777-4316-bccc-6f354f76ecb8，两条任务1350c22d-dbc8-4eaa-bd76-597adbda2e4a、d35987d4-9468-4f26-92d5-6d225924177e，14:03:33提交，4秒×2、2K9:16。首帧23f04309-1e9d-4f73-9a52-a076f63cb2d0、尾帧56cc8dc7-4b48-4de8-86f4-1777f3cc4b71均为已有COS first_frame资产；供应商请求含first_frame及last_frame两种角色。报价96积分、两条各预扣48。当前已消费671，可用233、待结算96，合计占用767/1000；未把在途任务记为验收通过。

秘塔页面新增任务2099740881031602176、2099740884532531200，每条2K4秒68H3积分，当前均处理中；页面共12条、827.5H3积分（包含在途），与成功消费分开看。保守供应商预算占用提高至70/100元（非实际现金总账），待最终计费核对。原始证据148-tail-2k-submitted.json；管理端仍待用户重新登录。PR #109继续更新，本节不是全业务完工声明。
...
## JOINT-RETEST-20260915 · 2026-09-15 13:30 最新增量

R52—R63增量在独立worktree开发；真实参考生视频12秒与四秒对照已生成、播放、归档并三端核账。累计消费623积分，可用377/冻结0；秘塔本轮691.5H3积分，其他供应商累计人民币仍待统一核对。人物外观跟随参考图，但背景/构图/字幕保持不合格，严格替换质量未通过。前端1472项、后端36项PG专项及静态门通过；完整分片与修正证据见验收文档。本轮尚待PR/CI/正常合并，不标整体业务完成或生产可用。详细逐项整改、授权迁移及剩余清单见[联调证据](evidence/BUSINESS-REMEDIATION-20260914.md)13:30节。
