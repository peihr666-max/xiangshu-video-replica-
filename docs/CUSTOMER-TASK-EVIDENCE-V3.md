...
## CW-030 — 收敛各类Worker的PG调度与恢复差额（2026-09-11）

分支 `feat/customer-v3-cw030-worker-pg-convergence`，基线 `origin/main@8ab85c7`、收尾 rebase 至 `d49f851`（CW-060 #27 后）。两处生产改动（均在任务文件清单内）：①`generation_worker.py` 物理删除 `run_sqlite_worker_round`/`run_forever` SQLite 轮询入口与孤儿导入，正式 Worker 仅剩 `run_pg_worker_round`/`run_forever_pg`，`main()` PG 模式后保留防御性 RuntimeError；SQLite 业务核心 `run_worker_once` 生产不可达，按 CW-042 分工保留；②`generation.py` `mark_task_submission_uncertain` 补迟到标记守卫——UPDATE 增加 `WHERE status IN ('SUBMITTING','RUNNING')` + `RETURNING`，仅真实在飞转换才释放用户 slot，堵住「同一任务迟到重复信号把替代任务正在使用的 slot 二次释放、每用户 running≤1 被击穿」的恢复差额（mark 侧补齐 P1-5 reconcile 同型守卫）。新增 `tests/test_cw030_worker_pg_matrix.py` 29 用例（专用库 cw030_worker_matrix_test，allowlist 已登记）：独立任务 5（同抢互斥、过期→UNCERTAIN+slot 释放且不盲重提、迟到信号零扰动、预置 RESERVE→Worker SETTLE 账本精确=[RESERVE,SETTLE]、crash 后 claim 持久 SUBMITTING 重启不盲重提）、首帧 5（同抢、attempt≥3 过期 UNCERTAIN、checkpoint 过期恢复 attempt 1→2 换主、陈旧 fail 零覆盖、提交围栏 UNCERTAIN/FAILED 分叉）、联系表 2（同抢+过期 UNCERTAIN）、人物图 3（SKIP LOCKED、剩余 attempts 过期回收、耗尽 fail-closed CHARACTER_LEASE_EXPIRED）、源帧 3（同抢、SOURCE_FRAME_TASK_RECOVERY_REQUIRED 仅人工恢复、质检配置缺失降级 run_pg_worker_round 真 PG 出片——移植自被删 SQLite 入口的行为）、文案改写 5（互斥、过期提交不重调 Provider、504→UNCERTAIN 不自动重试、陈旧 complete 抛 lease lost 且迟到结果零覆盖、Worker 真 PG 结算仅 _request_deepseek 造假）、ASR 5（互斥、提交前过期回 PENDING 重领、在飞过期 UNCERTAIN、Provider 失败终结不盲重提、audio_object_key 回执跨重试存活）、混合队列+两设备 2（H3+独立 3 用户 ≤8 轮全服务+slot 归零；同用户第二设备不得把 running slot 从 1 变 2）。回归：crash_recovery+fairness+wallet_billing+db_pg **130 passed**、independent+cw029+fencing **85 passed** 全绿（mark 守卫 live/sweep 行为不变）；oral_domain 38 errors 全为 MediaToolUnavailable（ffmpeg 缺失，CW-031/055/056 证据同口径，CI Linux 分片有 ffmpeg）；test_source_frames 21 passed/1 skipped、test_generation 受影响用例 1 passed（被删入口调用改 run_worker_once 直驱）；ruff/format/mypy strict/secrets 全绿。按 CW-002 登记限制：公平队列覆盖 H3/独立/口播，图片/文案/ASR 无公平机制本轮不扩展；跨口播完整轮转不实现；人物图/源帧无 UNCERTAIN 语义按各自真实机制断言；口播 Provider 超时维持传输中断形态口径；CW-004 阈值定标归 CW-045/047，生产负载归 CW-047。逐项断言与 §14 记录见 `docs/evidence/CW030-EVIDENCE.md`。
