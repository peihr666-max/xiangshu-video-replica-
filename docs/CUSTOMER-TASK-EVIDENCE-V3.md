...
继续原分支与 PR #109。单条视频号经明确授权完成 COS 归档，后台实际播放超过 23 秒且无解码错误；另经授权设置本机首页展示，客户前端验收因会话再次失效仍未完成。新增 R74：右下角提示 3 秒自动关闭，同文案重新触发重置计时，卸载清理计时器；TypeScript、Biome 与既有工作台 97 项测试通过，开发预览确认提示出现后自动消失。原全量验证为前端 1482 项、PG 2199 通过/1 跳过；新提示改动待新提交 CI。详情见[任务证据](evidence/BUSINESS-REMEDIATION-20260914.md)。状态仍为进行中，未合并、未部署。
