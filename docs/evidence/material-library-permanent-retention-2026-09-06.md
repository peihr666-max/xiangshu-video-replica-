...
素材库已形成服务端列表、按 ID 恢复、通用上传、预览下载、口播音频引用和基础管理闭环。口播页可以直接上传 MP3 并把真实资产 ID 写入草稿。新 H3 成片会归档到当前应用存储，归档成功后才结算；归档失败保留供应商结果并有限重试。代码不再创建项目素材与成片的 180 天自动过期规则，保存 COS 配置时会读取现有生命周期配置，仅移除历史媒体 180 天规则；其他业务规则原样保留。
...
- `server/app/permissions.py` / `oral.py`：素材 owner 隔离、口播输入类型/状态/归属校验和口播结果本人读取；全局查看权限不能转化为使用他人音频生成的权限。
- `client/src/studio/ContentPages.tsx`：云端列表、通用上传、类型/来源/名称筛选、当前页卡片预览、下载、重命名、分组、隐藏和已闭环的音频口播复用。
...
## 用户隔离与联动边界（2026-09-07 补充）

- `employee` / `customer` 的列表、按 ID 恢复、修改、隐藏、下载和口播引用都按 owner 在服务端校验；给自己写一条他人素材偏好不会获得访问权。
- `admin` 可以跨用户查看素材，并以管理员自己的偏好重命名或隐藏视图；不能替其他用户写入/完成上传，也不能把他人音频用于自己的口播任务。
- `auditor` 可以读取审计范围内的元数据和历史直出结果预览，不可上传、下载私有存储对象、修改、隐藏、复用或创建口播任务。
- 素材接口只返回已经有真实后端消费链路的 `oral_audio` 用途。独立图片/视频创作仍属于 C2，在接口落地前不展示“原画面/首帧/尾帧/参考生视频”按钮。
- 私有视频代理支持单段 HTTP Range，浏览器预览不再要求应用一次性读取整个 COS 视频对象。
- 通用素材先上传到用户目录下的临时对象，校验内容、大小和哈希后再复制到含内容哈希的正式对象并更新数据库；旧上传地址继续有效时也只能改写临时对象，并发完成也不会覆盖另一份正式内容。
