# CW-001 — 整合候选、补丁来源与发布基线决议（已签认，owner 2026-09-09）
...
| P3 | W6 清理组解锁（039 死闸门拆解） | ①CW-042 拆 042-a（入口 fail-closed，pre-GA 可做、不含 CW-039）/042-b（物理删除，保留 CW-039 前置，GA 后清理批次）；②CW-040/041 pre-GA 范围=P1 入口 fail-closed；③CW-039 拆「手册定版（纯文档可先做）/真实演练（维持 GA 冻结）」，不记 N/A | CW042-SCOPE-INVENTORY §2：039 前置全为 GA 触发/环境阻塞，pre-GA 结构性无法闭环；P1 松绑对象是 CW-044，不构成物理删除授权；本决策与 P1 同构（先关入口，物理清理另起批次） |
...
| 集成负责人（P3 W6 清理组解锁） | 已签认（owner hlong026）：2026-09-12 会话明示授权按 CW042-SCOPE-INVENTORY §2.4 方案实施并统一批次开发（单 worktree、先开发后评审提 PR）；决策正本 docs/evidence/COORD-W6-UNBLOCK-20260912.md；实施证据 docs/evidence/CW042A-EVIDENCE.md、CW041-EVIDENCE.md、CW040-EVIDENCE.md | 2026-09-12 |
