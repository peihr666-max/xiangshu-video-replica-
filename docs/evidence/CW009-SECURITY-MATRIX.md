...
| CSV | `test_admin_customer_routes.py::test_customer_code_materials_never_reach_control_csv_exports`（**CW-009 新增**）+ ~~test_internal_admin.py · test_control_reconciliation_and_csv_are_read_only~~（CW-042-b 退休；对账 CSV 只读由 cw029/cw059 PG 矩阵对账面覆盖） + `test_admin_customer_routes.py::test_customers_csv_export_is_audited_and_filtered` | 明文码与 digest 不得进入 customers.csv；merchant-secret 不进对账 CSV |
...
| 旧设备迟到写入 | `test_customer_sessions.py::test_logout_never_changes_another_devices_session`、`test_customer_fencing.py::test_cluster_probe_detects_heartbeat_from_a_displaced_session_epoch`、`test_customer_fencing.py::test_verify_fences_a_lease_snapshot_that_was_shortened`、`test_customer_fencing.py::test_fenced_transaction_fences_a_stale_snapshot_and_leaves_no_write` | heartbeat/logout/业务写；同一身份正常续租允许，缩短租期仍拒绝 |
...
- 用例（CW-042-b 更正：原 test_payments.py SQLite 回调套件随内部 lane 退休，不变量由 CW-029 TEST-PG 支付矩阵等价承载——伪造回调零副作用/重复幂等/跨单拒绝/乱序 409/CLOSED 保留对账/价格快照，见 test_cw029_billing_pg_matrix.py；签名排序由 test_zpay.py 承载）：`test_cw029_billing_pg_matrix.py::test_forged_callbacks_have_zero_side_effects`、`test_cw029_billing_pg_matrix.py::test_paid_notify_credits_once_and_duplicate_out_of_order_is_idempotent`、`test_cw029_billing_pg_matrix.py::test_provider_trade_no_cannot_credit_two_users_orders`、`test_cw029_billing_pg_matrix.py::test_customer_closed_order_keeps_lineage_and_late_callback_reconciles`、`test_cw029_billing_pg_matrix.py::test_order_price_snapshot_survives_runtime_price_change`、`test_zpay.py::test_zpay_signature_matches_official_sorting_and_unencoded_values`。
