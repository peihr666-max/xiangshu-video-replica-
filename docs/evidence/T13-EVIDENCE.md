...
Task exit gate: *user、wallet、activation、slot1、PAID order、CHARGE、session 全有或全无* (task list §3 T13) + ACT-05 gate (same) + ACT-06 gate: *同一码、同用户名和同设备的 100 并发测试 — 仅一个成功事实、一个首充、一个当前 session；SQLite/单连接测试不能作为并发证据* — delivered as `server/app/activation_code_routes.py` (frozen name, mounted in `main.py`) over migrations `028_customer_devices_and_activations` + `029_customer_sessions_and_idempotency` (both frozen names, chained off the then-current head 031), with 32 fail-first tests (`server/tests/test_activation_code_routes.py` 24 contract cases incl. five session-review + three PR #44-review regressions + `server/tests/test_customer_activation.py` 5 concurrency cases on dedicated migrated PG fixture databases + three PR #44-review trigger cases in `test_activation_code_schema.py`):
...
## 4. Test Coverage (32 red → green + 2 adapted)

| Group | Cases |
| --- | --- |
| Contract (`test_activation_code_routes.py`, 24) | atomic happy path (201, full chain incl. wallet balance, epoch-1 lease); request-id echo; missing Idempotency-Key 400; unified 400 ×7 (unknown/malformed/expired/suspended/revoked/active/generated-shape); same fingerprint second code 409 `USER_ALREADY_ACTIVATED`; same key + same body replays identical identity (replay header, original request id restored); same key + different body 409; SQLite fail-closed 503; log scan: no plaintext code/token in any captured record; **PR-review regressions** — naive batch expiry answers the unified 400 (never a TypeError 500), envelope row shape after success (ciphertext/key_version/recovery deadline present, purged_at NULL), recovery-window env override (60 s → stored deadline follows), expired recovery window refuses replay (409, no replay header), same key + whitespace-padded body still replays (normalized request hash); **PR #44 review regressions** — CORS preflight permits `Idempotency-Key`/`X-Request-Id` and the actual response exposes `X-Request-Id`/`X-Idempotent-Replay`; rotation-window fingerprint check (V1-bound device + V2 added → second code still 409, one user/CHARGE); envelope scoped under V1 still replays after V2 is added (same device/session tokens, one envelope, one CHARGE) |
...
| Schema adaptation + triggers (`test_activation_code_schema.py`, 5) | one-shot uniqueness test now seeds the slot-1 `customer_devices` row (028 attached 027's deferred `first_device_id` FK); downgrade-guard test walks the longer chain (029 → 028 refuses with an activation fact via "cannot downgrade 028"; after removing the fact four steps reach 026); **PR #44 review regressions** — both 029 triggers exist on the tables (epoch-monotonic + append-only), an epoch decrease is rejected by the trigger (`must never decrease`), UPDATE/DELETE on the audit table is rejected (`append-only`) |
...
| `server/app/main.py` | mount `customer_activation_router`; CORS `allow_headers` adds `Idempotency-Key`/`X-Request-Id` and `expose_headers` adds `X-Request-Id`/`X-Idempotent-Replay` (PR #44 review P1; `OPTIONS` joins the allowed methods) |
| `server/tests/test_activation_code_routes.py` | new: 24 fail-first contract cases (16 original + 5 session-review + 3 PR #44-review regressions) on a dedicated migrated fixture DB |
...
| `server/tests/test_activation_code_schema.py` | two T10 cases adapted to the 028/029 chain (slot-1 seed row, downgrade walk) + three PR #44-review trigger cases |
...
780 passed                      # full suite on the PG fixture (zero regressions; the three
                                 # previously-skipped network-dependent cases also ran green)
...
Three review comments landed on the PR (2 P1, 1 P2), each addressed substantively with red → green regressions before the merge:

1. **P1 — CORS blocked the mandatory idempotency header**: the WebView/browser client must send `Idempotency-Key` (plus `X-Request-Id`), but `app.main`'s `CORSMiddleware.allow_headers` listed neither, so the preflight failed before the handler ran and first activation was unusable from the intended client. Fixed: both headers join `allow_headers` (`OPTIONS` joins the allowed methods), and `expose_headers` adds `X-Request-Id`/`X-Idempotent-Replay` so browser JS can read the replay marker and echoed request id; regression test covers the preflight (200 + allowed headers) and the actual response (expose headers present on a validation-error pass-through).
2. **P1 — fingerprints only checked under the highest HMAC version**: during a rotation window (V1 retained, V2 added) the binding check computed only the V2 digest, so a device bound under V1 was invisible to the lookup *and* to the partial unique index — the same physical device could redeem a second code and receive a second user, wallet and first charge; an envelope scoped under V1 also lost its replay path. Fixed: the route computes the fingerprint digest under *every* configured device-domain version; the bound check queries `fingerprint_hmac = ANY(%)`, the envelope lookup walks every version's scope (highest first, replaying under the found scope's AAD), while new rows always carry the highest version's digest; regressions: rotation-window second activation → 409 with one user/CHARGE, and a pre-rotation envelope still replays after V2 is added (same device/session tokens, one envelope, one CHARGE).
3. **P2 — the epoch-monotonicity trigger was never installed**: 029 defined the `_EPOCH_MONOTONIC_TRIGGER` DDL and its downgrade removed the trigger, but `_create_session_state()` never executed the DDL — upgraded databases silently allowed `customer_session_state.session_epoch` to decrease, removing the very invariant that fences out a stale device after a switch. Fixed inside the same unpublished revision (append-only rule permits pre-release correction, the 028→031 rename precedent): `op.execute(_EPOCH_MONOTONIC_TRIGGER)` now runs in `_create_session_state()`; regressions: both 029 triggers exist on the tables, an epoch decrease is rejected (`must never decrease`), and UPDATE/DELETE on the audit table is rejected (`append-only`).
...
验证命令与通过数：test_activation_code_routes 24 passed + test_customer_activation 5 passed + test_activation_code_schema 13 passed；全量 780 passed（PG fixture，含此前 3 个 skip 的网络依赖用例）；ruff/format/mypy 全绿；client workspace 324 passed；biome e2e clean；cargo fmt+check clean；secret 扫描等价模式 clean
