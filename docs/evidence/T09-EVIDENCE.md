...
Enforced twice: in `bootstrap.main()` (before ready-check/pool warm-up) and in the API `_lifespan` startup (uvicorn aborts on lifespan errors). SQLite/missing-DSN rejection was delivered by T05 (`validate_customer_production`) — originally only on the bootstrap/Worker paths; **M1 review H1 (2026-08-23) fixed the gap where a direct `uvicorn app.main:app` boot skipped it: `_lifespan` now resolves the database config and runs `validate_customer_production` as well (regression locks: `test_api_lifespan_fails_closed_on_sqlite_lane_in_customer_production` etc. in `tests/test_admin_auth.py`)**. The gate stacks on top so *all five* T09 exit-gate categories refuse to boot on every entrypoint. All violations are reported together in one fail-closed message. At runtime, `control_auth.get_control_user` additionally rejects customer-production requests with 403 `LEGACY_CONTROL_IDENTITY_FORBIDDEN` (defence in depth).
