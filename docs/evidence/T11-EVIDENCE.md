...
| `server/tests/test_activation_code_service.py` | new: 22 fail-first cases (12 unit, 10 PG on a dedicated migrated fixture database) |
...
## PR #42 Review Fixes (Codex)

External review on PR #42 (chatgpt-codex-connector, commit `c29a6a4`) left 2 P1 + 1 P2; all three were substantively resolved (fix commit on the same PR):

1. **P1 — generation budget race (accepted, fixed)**: the budget check read `quantity` and the pre-insert count without locking, so two API instances could jointly overshoot a batch's frozen quantity. `generate_batch_codes` now takes the batch row with `SELECT … FOR UPDATE`, serializing check-then-insert across instances; a red test proves a concurrent generator waits on the lock (`SET LOCAL lock_timeout` → `LockNotAvailable`) and, after the first transaction commits, fails closed with "budget exceeded".
2. **P1 — private-COS export delivery (scope clarified with code-level hand-off)**: ACT-03's private-COS object delivery is layered on by **T36 / COS-01** per the task-list boundary: T11 depends only on T10, its exit gate is "熵测试、日志扫描、导出下载审计" (no COS), `VIDEO_REPLICA_COS_*` env is registered as a T36/T40 placeholder, and the already-merged revision 027 (reviewed in PR #41) freezes the exports-table ciphertext carrier. Pulling the COS client into T11 would invert the task graph (T12/T13 depend on T11 but sit before the T34→T36 infrastructure chain). The one-time/audited download semantics implemented here stay authoritative; `create_batch_export` / `fetch_export_package` docstrings now state the storage carrier and the T36/COS-01 hand-off explicitly.
3. **P2 — inexact AEAD key length (accepted, fixed)**: the resolver accepted any key ≥ 32 bytes, but `AESGCM(key)` only takes 16/24/32-byte keys — a 48-byte key would fail later as a raw `ValueError`. `export_aead_key` now validates **exactly 32 decoded bytes** (AES-256) at configuration resolution; a red test rejects a 48-byte key with "exactly 32 bytes", and the env example contract was updated.

## Pre-PR Session-Internal Review Fixes
...
22 passed                       # 12 unit + 10 PG integration (incl. all review locks)
$ uv run python -m pytest tests -q
710 passed, 2 warnings          # full suite on the PG fixture (zero regressions)
...
Owner / Reviewer：后端/安全（Agent 执行）/ 会话内代码评审（0 P1、2 P2 + 5 P3 全部实质修复）+ PR #42 chatgpt-codex-connector（2 P1 + 1 P2 全部实质处置）
...
改动文件：server/app/activation_code_service.py（新增，460 行：规范化/CSPRNG 生成/掩码、版本化 HMAC/AEAD 密钥解析、轮换窗摘要、六态转移矩阵、AES-GCM 信封、批次生成+一次性审计导出持久化）、server/tests/test_activation_code_service.py（新增 22 用例：12 单元 + 10 PG 集成，专用迁移 fixture 库）、deploy/customer.env.example（登记 T11 双密钥族+生成命令+_V2 轮换注释）、docs/evidence/T11-EVIDENCE.md、任务与证据账本
...
验证命令与通过数：test_activation_code_service 22 passed；全量 710 passed（PG fixture）；ruff/format/mypy 全绿；CI 三门禁见 PR
