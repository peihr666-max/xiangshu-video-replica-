...
按用户澄清，显示框以生成参数选择的画面比例为准，而不是上传图片原始比例。新草稿默认 9:16，已保存的比例不迁移覆盖；文/图首尾帧、参考缩略图、右侧预览和空占位联动。共享图片显示使用 contain 与现有虚化衬底，首帧候选跟随其图片画幅设置（历史候选保留历史画幅）；只有“自动/沿用原图”读取原始尺寸。

- TypeScript 与修改文件 Biome 通过。最终完整前端 106 文件 / 1631 项通过（image-ratio-final.log）。首轮 1629 通过 / 2 失败：图片重签测试触发了装饰背景图而非主图，调整 DOM 顺序保留主图优先后通过；凭据切换用例首次出现时序失败，未修改认证代码，专项及最终全量均通过。专项复测 105 项通过。日志位于宿主 outputs/creation-ui-gate。
- 浏览器核对参考页 9:16 框 254.7×452.8、16:9 框 715.8×402.6、1:1 框 452.8×452.8、21:9 框 715.8×306.8；缩略图同步。
- 文/图页首尾帧 9:16 框均为 90×160（尾帧为空），右侧 254.7×452.8；参数下拉框底部 805px，底栏顶部 812px，没有遮挡。390×844 下页面内容宽 386px，无横向溢出。
