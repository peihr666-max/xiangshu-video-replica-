...
新设备发起配对时产生紧急通知并进入设备审批。会话被替换或凭据被撤销时，既有强制登录提示继续负责即时阻断，持久化安全通知负责留下用户可见记录；通知不能包含设备 token、完整指纹或激活码。
...
| `security.session.replaced` | 用户会话被新会话替换 | URGENT | `session_epoch:{user_id}:{replaced_epoch}:REPLACED` | 查看账号安全 | 否 |
...
| `NTF-WP3` 钱包与安全事件 | M | WP1 已合 main | 充值成功、低余额周期、设备配对、会话替换 | 重复支付回调通知一次；金额/安全信息脱敏；既有账务和 Session 原子性不回归 |
...
- A→B→A 账号切换、退出、被顶号、角色变更、组件卸载后，旧响应不污染当前账号。
