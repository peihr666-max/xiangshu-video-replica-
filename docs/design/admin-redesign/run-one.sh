#!/usr/bin/env bash
# 用本地 Codex CLI 的 Image Gen 能力生成一页管理后台效果图
# 用法: bash run-one.sh <prompt-number-and-slug>   例: bash run-one.sh 01-overview
set -euo pipefail
...
【图像提示词开始】
WRAPPER
  cat "$HERE/prompts/00-shared-style.md"
  echo
  cat "$PROMPT_FILE"
  echo
  echo "【图像提示词结束】"
